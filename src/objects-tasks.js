...
function removeProperties(obj, keys) {
  ...
  Object.keys(answer).forEach((key) => {
    if (keys.includes(key)) delete answer[key];
  });
...
function compareObjects(obj1, obj2) {
  ...
  Object.keys(obj1).forEach((item) => {
    if (obj1[item] !== obj2[item]) answer = false;
  });
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject).forEach((item) => {
    lettersObject[item].forEach((pos) => {
      answer[pos] = item;
    });
  });

  return answer.filter((item) => Boolean(item)).join('');
}
...
function sellTickets(queue) {
  ...
  queue.forEach((item) => {
    ...
    if (item === 100) {
      ...
      if (banknotes[25] < 0 || banknotes[50] < 0) answer = false;
    }
  });
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    if (answerMap.has(key)) {
      const valueArr = answerMap.get(key);
      valueArr.push(valueSelector(item));
      answerMap.set(key, valueArr);
    } else {
      const valueArr = [valueSelector(item)];
      answerMap.set(key, valueArr);
    }
  });
